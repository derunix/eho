...
— Извлекай максимально ПОЛНО: персонажи, места, предметы, магию, отношения, состояния, события, причины, последствия, локальные открытия сцены.
...
def deduplicate_knowledge(facts: list[dict]) -> list[dict]:
    ...
    for f in facts:
        ...
        exact_key = text_hash(f"{subject_norm}:{fact_norm}")
...
def knowledge_candidate_score(new_item: dict, existing_item: dict) -> float:
    ...
    if facts_look_duplicate(new_fact, existing_fact):
        score += 10.0

    new_clause = leading_fact_clause(new_fact)
    existing_clause = leading_fact_clause(existing_fact)
    if new_clause and existing_clause and new_clause == existing_clause:
        score += 5.0
...
— Не придумывай.
— Извлекай максимально полно: и знания о мире, и важные локальные события сцены.
